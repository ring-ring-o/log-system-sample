...
アプリは Collector にだけ送ればよく、**収集先の変更・追加（SigNoz→Grafana等）でアプリを変更しない**。再試行・バッチ・バックプレッシャ・属性付与（`resource`）・将来の tail sampling を Collector に集約できる（関心の分離）。
...
- Collector に **tail-based sampling**（エラー/高レイテンシ trace を優先保持）。
